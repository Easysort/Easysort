...
class U2NETP():
    def __init__(self,in_ch=3,out_ch=1):
        ...
        self.side1 = Conv2d(64,out_ch,3,padding=1)
        self.side2 = Conv2d(64,out_ch,3,padding=1)
        self.side3 = Conv2d(64,out_ch,3,padding=1)
        self.side4 = Conv2d(64,out_ch,3,padding=1)
        self.side5 = Conv2d(64,out_ch,3,padding=1)
        self.side6 = Conv2d(64,out_ch,3,padding=1)
...
# Classifier built on U2NETP backbone: outputs a single logit for presence detection
class U2NETPClassifier():
    def __init__(self, in_ch: int = 1):
        # Use small U2NETP backbone with configurable input channels
        self.stage1 = RSU7(in_ch,16,64)
        self.stage2 = RSU6(64,16,64)
        self.stage3 = RSU5(64,16,64)
        self.stage4 = RSU4(64,16,64)
        ...
        # classification head: global avg+max pool -> concat -> 128 -> 1
        self.cls = Linear(64*2, 1)

    def __call__(self, x: Tensor) -> Tensor:
        # Accept (B,C,H,W) or (B,H,W)
        if len(x.shape) == 3:
            x = x.unsqueeze(1)
        hx = x
        ...
        hx6 = self.stage6(hx)
        hx6up = hx6.interpolate(hx5.shape, mode="linear")

        # decoder
        hx5d = self.stage5d(hx6up.cat(hx5,dim=1))
        hx5dup = hx5d.interpolate(hx4.shape, mode="linear")

        hx4d = self.stage4d(hx5dup.cat(hx4,dim=1))
        hx4dup = hx4d.interpolate(hx3.shape, mode="linear")

        hx3d = self.stage3d(hx4dup.cat(hx3,dim=1))
        hx3dup = hx3d.interpolate(hx2.shape, mode="linear")

        hx2d = self.stage2d(hx3dup.cat(hx2,dim=1))
        hx2dup = hx2d.interpolate(hx1.shape, mode="linear")

        hx1d = self.stage1d(hx2dup.cat(hx1,dim=1))

        # global average and max pooling on hx1d (B,64,H,W)
        H, W = hx1d.shape[-2], hx1d.shape[-1]
        xav = hx1d.avg_pool2d(kernel_size=(H, W)).reshape((hx1d.shape[0], hx1d.shape[1]))
        xmax = hx1d.max_pool2d(kernel_size=(H, W)).reshape((hx1d.shape[0], hx1d.shape[1]))
        feats = xav.cat(xmax, dim=1)  # (B, 128)
        logits = self.cls(feats)
        return logits.reshape((logits.shape[0], 1))

    @property
    def params(self):
        # recursively collect weights/bias from submodules
        params = []
        seen = set()
            ...
            for attr in ("weight", "bias"):
                if hasattr(obj, attr):
                    ...
                    if isinstance(t, Tensor):
                        params.append(t)
            # recurse into dict or tuple members
        ...
        collect(self)
        # ensure unique tensors
        uniq = []
        seen_t = set()
        for p in params:
            if id(p) not in seen_t:
                uniq.append(p)
                seen_t.add(id(p))
        return uniq