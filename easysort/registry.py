...
class Registry:
    ...
    def POST(self, key: str, data: dict|bytes|np.ndarray|list) -> None: {dict: self._post_json, list: self._post_list, bytes: self._post_bytes, np.ndarray: self._post_numpy}[type(data)](Path(key).with_suffix(""), data)
